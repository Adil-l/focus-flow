    ...
    <div className="bg-white/[0.04] rounded-xl p-4 border border-white/[0.05] space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h4 className="text-xs font-bold text-white/60 uppercase tracking-widest flex items-center gap-2">
          <ImageIcon size={12} /> Live Wallpapers
        </h4>
      ...
      {WALLPAPER_SEARCH_ENABLED ? (
        ...
        <p className="text-[11px] text-white/40 leading-relaxed">
          Curated random photos. Add a free <span className="text-white/60 font-bold">VITE_UNSPLASH_ACCESS_KEY</span> or{' '}
          <span className="text-white/60 font-bold">VITE_PEXELS_API_KEY</span> in <span className="font-mono">.env</span> to unlock keyword search.
        </p>
      )}
      ...
      {error && items.length === 0 ? (
        <div className="py-8 text-center text-[12px] text-white/40">
          Couldn't load wallpapers. Check your connection and try again.
        </div>
      ) : (
        <div className="grid grid-cols-3 gap-2">
          {items.map(w => (
            <button
              ...
              title={w.author ? `Photo by ${w.author}` : 'Apply wallpaper'}
      ...
      <div className="flex items-center justify-between gap-2 pt-1">
        ...
        >
          Photos via {WALLPAPER_PROVIDER} <ExternalLink size={10} />
        </a>
      </div>
    </div>
  );
