...
function createNoiseNode(ctx: AudioContext, type: string): AudioBufferSourceNode {
  ...
  else if (type === 'brown') { let last = 0; for (let i = 0; i < bufferSize; i++) { data[i] = (last + 0.02 * (Math.random() * 2 - 1)) / 1.02; last = data[i]; data[i] *= 3.5; } }
  else { let b0=0,b1=0,b2=0,b3=0,b4=0,b5=0,b6=0; for (let i = 0; i < bufferSize; i++) { const w = Math.random()*2-1; b0=0.99886*b0+w*0.0555179;b1=0.99332*b1+w*0.0750759;b2=0.96900*b2+w*0.1538520;b3=0.86650*b3+w*0.3104856;b4=0.55000*b4+w*0.5329522;b5=-0.7616*b5-w*0.0168980; data[i]=(b0+b1+b2+b3+b4+b5+b6+w*0.5362)*0.11; b6=w*0.115926; } }
...
export default function SoundsPanel({ ambientSound, ambientVolume, onSoundChange, onVolumeChange }: SoundsPanelProps) {
  ...
  const playSound = (id: string) => {
    ...
    if (['white', 'brown', 'pink'].includes(id)) {
      const ctx = new AudioContext(); const gain = ctx.createGain(); gain.gain.value = ambientVolume; gain.connect(ctx.destination);
      const node = createNoiseNode(ctx, id); node.connect(gain); node.start();
      ctxRef.current = ctx; noiseRef.current = node; gainRef.current = gain;
    } else {
      const sound = SOUNDS.find(s => s.id === id);
      if (sound?.url) { const audio = new Audio(sound.url); audio.loop = true; audio.volume = ambientVolume; audio.play().catch(() => {}); audioRef.current = audio; }
    }
  };

  useEffect(() => { if (audioRef.current) audioRef.current.volume = ambientVolume; if (gainRef.current) gainRef.current.gain.value = ambientVolume; }, [ambientVolume]);
  useEffect(() => () => stopAll(), []);
  ...
  return (
    <motion.div
      ...
      className="glass-panel p-5 w-[380px] max-h-[420px] flex flex-col"
    >
      {/* Tabs */}
      ...
      {tab === 'music' && (
        <div className="flex-1 flex flex-col items-center justify-center text-center py-8">
          <p className="text-white/50 text-sm mb-3">Connect your music player</p>
          <button className="px-4 py-2 rounded-full bg-[#1DB954] text-white text-sm font-medium">
            Load Spotify Player
          </button>
        </div>
      )}
      ...
      {tab === 'playlists' && (
        <div className="flex-1 space-y-2">
          {['Lofi', 'Rainy Day Lofi', 'Paris Café', 'Jazz Vibes', 'Ambient'].map(p => (
            <button key={p} className="w-full text-left p-3 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] text-sm text-white/80 transition-all">
              🎵 {p}
            </button>
          ))}
        </div>
