...
export default function AchievementsPanel({ xp, unlockedAchievements }: AchievementsPanelProps) {
  const [filter, setFilter] = useState<'all' | 'unlocked' | 'locked' | 'leaderboard'>('all');
        ...
        <AnimatePresence mode="wait">
          {filter === 'leaderboard' ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
              {onlineUsers.length === 0 && <p className="text-center text-white/20 py-10">No focusers online</p>}
              {onlineUsers.map((u, i) => (
                <div key={u.user_id} className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.04]">
                  ...
                  <div className="flex-1 text-sm text-white/80">{u.email}</div>
